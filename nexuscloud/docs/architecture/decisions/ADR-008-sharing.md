...
- Fuera de esta pasada, documentado explícitamente: subida con permiso de escritura dirigida a un usuario/grupo concreto (solo enlaces la soportan; resuelto después en [ADR-035](ADR-035-subida-a-carpeta-compartida.md)); §38 (Subida Anónima) sigue totalmente separado -- es un modelo de autorización distinto (activación explícita del admin, foco anti-abuso), no una variante de un share normal (resuelto después en [ADR-039](ADR-039-subida-anonima.md)).
