...
- **Verificación limitada, aceptada explícitamente, más que ZFS**: `parseBtrfsSubvolumes`/`parseBtrfsMountpoints` están probados con 7 tests unitarios contra texto sintético basado en el formato documentado de `btrfs-progs` (una línea, varias líneas, una línea malformada, salida vacía, un `path` con espacios embebidos, montajes filtrados/no encontrados), pero nunca contra una salida real de `btrfs subvolume list` -- instalar Btrfs y crear un subvolumen real en el entorno de desarrollo exigiría privilegios de montaje/bloque que el sandbox de esta sesión ya deniega para operaciones equivalentes (mdadm, ADR-018; ZFS, ADR-021). Si el formato de texto de `btrfs subvolume list` cambiara de forma incompatible en una versión futura de `btrfs-progs` (posible: es texto para humanos, no una API estable como `-H -p` en ZFS), el fallo se manifestaría como líneas ignoradas silenciosamente (nunca un panic, por diseño) en vez de un error visible -- riesgo aceptado y documentado, no descubierto en producción sin más.
