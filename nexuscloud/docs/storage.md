...
- **Permisos**: `can_download`/`can_upload`. Un enlace puede ser de descarga, de subida o de ambas. Un share de usuario o de grupo es siempre de lectura y, sobre una **carpeta**, puede añadir la subida («lectura y subida», [ADR-035](architecture/decisions/ADR-035-subida-a-carpeta-compartida.md)): el destinatario sube desde «Compartido» (`POST /api/v1/shared-directories/{id}/files`), a la carpeta compartida o a cualquiera de sus subcarpetas, y el archivo queda en el árbol del propietario. **No sobrescribe un archivo existente** (409 si el nombre ya existe), quien sube no puede renombrar, borrar ni crear subcarpetas, y el límite de tamaño por archivo (`max_upload_size_bytes`) se aplica igual que en un enlace; si varios permisos coinciden, gana el más permisivo. Quién subió cada archivo queda en la auditoría (evento `upload` con `via: shared_directory`). La subida por enlace público sigue la misma regla: no sobrescribe un archivo existente (409).
