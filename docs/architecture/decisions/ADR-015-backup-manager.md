...
- Quedan explícitamente fuera de este slice, para ADRs/slices futuros: RAID (§12) y snapshots (§17) — detección del SO; backup incremental, programado/automático, cifrado, rotación/retención automática (§18/§173); otros destinos como "otro servidor NexusCloud" (SMB/NFS ya funciona hoy montado como carpeta local, sin código especial); un comando `backup verify` dedicado (la verificación ya ocurre dentro de `run` y `restore`); alertas (§58), dashboard (§56) y documentar la regla 3-2-1 (§19).
