...
| POST | `/api/v1/favorites` | sesión | `{resource_type: "file"\|"directory", resource_id}` → marca un favorito (§87, ADR-038), solo sobre el árbol propio (403 si el recurso no es tuyo). Favoritar dos veces el mismo recurso no es un error: devuelve el existente |
| GET | `/api/v1/favorites` | sesión | Favoritos propios resueltos (misma forma que `GET /files`, con `favorite_id` ya poblado); solo recursos activos, uno trasheado se oculta hasta que se restaura |
| DELETE | `/api/v1/favorites/{id}` | sesión, propietario | Quita un favorito propio (404 si no existe o no es tuyo, §198) |
| GET | `/api/v1/activity?limit=` | sesión | Feed de "Recientes" (§88, ADR-038): actividad reciente del usuario autenticado (subidas/descargas/borrados/movimientos/comparticiones), más reciente primero. `limit` por defecto 20, tope 100 — es un widget de dashboard, no el listado administrativo (`GET /audit`) |
