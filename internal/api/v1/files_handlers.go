...
func (h *Handlers) ListFiles(w http.ResponseWriter, r *http.Request) {
	...
	list, err := h.Files.List(r.Context(), u.ID, parentPath)
	if err != nil {
...
func (h *Handlers) Mkdir(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.Files.Mkdir(r.Context(), u.ID, req.ParentPath, req.Name); err != nil {
		writeFileError(w, err)
		return
	}
...
func writeFileError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, storage.ErrForbidden):
		writeError(w, http.StatusForbidden, "forbidden", "No tienes permiso sobre este archivo.")
	...
	case errors.Is(err, storage.ErrInvalidName), errors.Is(err, storage.ErrInvalidPath), errors.Is(err, storage.ErrPathEscapesRoot):
		writeError(w, http.StatusBadRequest, "invalid_request", "Nombre o ruta de archivo inválidos.")
	default:
